...
###  iysgvbxfiektink@midiharmonica.com:b~6vaKQY

twitch_miner = TwitchChannelPointsMiner(
    username="abssduo3",
    password="k8ki0lz1Z",           # If no password will be provided, the script will ask interactively
    ...
    logger_settings=LoggerSettings(
        ...
        telegram=Telegram(                                                          # You can omit or leave None if you don't want to receive updates on Telegram
            chat_id=5036976963,                                                      # Chat ID to send messages @GiveChatId
            token="5283707288:AAG38FAMsSd2j3lFI3wxhZOvSPRGvaJZOuI",                          # Telegram API token @BotFather
...
twitch_miner.mine(
    [
        Streamer("nikgewing", settings=StreamerSettings(make_predictions=True  , follow_raid=True , claim_drops=True  , watch_streak=True , bet=BetSettings(strategy=Strategy.SMART      , percentage=5 , stealth_mode=True,  percentage_gap=20 , max_points=234   , filter_condition=FilterCondition(by=OutcomeKeys.TOTAL_USERS,      where=Condition.LTE, value=800 ) ) )),
        Streamer("redroses11111", settings=StreamerSettings(make_predictions=True , follow_raid=True  , claim_drops=True , watch_streak=True , bet=BetSettings(strategy=Strategy.PERCENTAGE , percentage=5 , stealth_mode=True, percentage_gap=20 , max_points=1234  , filter_condition=FilterCondition(by=OutcomeKeys.TOTAL_POINTS,     where=Condition.GTE, value=250 ) ) )),
