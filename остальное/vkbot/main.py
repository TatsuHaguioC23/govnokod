...
TOKEN_VK = 'eec79a2a766ae5b8fc147173ca746d377e2d7cd91940b8be79d9b0b9c05e5b413f7f1b65cf1a2d82cc896'
TOKEN_TG = '5155465221:AAGS7_pxVYxRJzrq9tNCDIP4ROnRT3GgC7w'
...
async def start_command(message: types.Message):
  await message.answer('Добро пожаловать, это бот, для сбора информации об друзьях ВКонтакте'
...
async def give_info(message: types.Message, state: FSMContext):
  ...
  if 'https://vk.com/' not in link:
    ...
    main_id_data = {'user_ids': link_id,
                    'fields': 'personal',
                    'access_token': TOKEN_VK,
                    'v': '5.131'}

    response = requests.post('https://api.vk.com/method/users.get', data = main_id_data)
    ...
    id_friend_data = {'user_id': main_id,
                      'access_token': TOKEN_VK,
                      'v': '5.131'}

    response = requests.post('https://api.vk.com/method/friends.get', data = id_friend_data)
    ...
    for id_person in id_person:
      info_friends_data = {'user_ids': id_person,
                          'fields': 'bdate, contacts, city, schools, photo_max_orig',
                          'access_token': TOKEN_VK,
                          'v': '5.131'}

      response = requests.post('https://api.vk.com/method/users.get', data = info_friends_data)
      info = response.json()
    ...
    all_cities = all_cities.replace('Counter({', '').replace('})', '').replace("'", "").replace(', ', '\n')

    all_schools = str(Counter(all_schools))
    all_schools = all_schools.replace('Counter({', '').replace('})', '').replace("'", "").replace(', ', '\n')
    ...
    await message.answer('Неверно указано ссылка')
